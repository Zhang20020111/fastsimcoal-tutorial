...
k<-(grep("COMPLEX PARAMETERS",est))-(grep("//all Ns are",est)+1)

# test k output - zhangjiaqi
cat("Number of parameters k:", k, "\n")
cat("MaxEstLhood:", bestlhoods$MaxEstLhood, "\n")

# Calculate AIC
AIC<-2*k-2*(bestlhoods$MaxEstLhood/log10(exp(1)))

# test AIC output - zhangjiaqi
cat("Calculated AIC:", AIC, "\n")
